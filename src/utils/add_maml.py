...
class Add_MAML:
    ...
    def add_maml(self, args, ensemble_model, moving_avg, meta_generators):
        ...
        if len(ensemble_model.model) < 2:
            ...
            print(ensemble_model.optimizers.param_groups[0]['params'][-2])
        ...
        args.num_maml +=1
        moving_avg.values = []
        moving_avg.threshold_count = 800
        args.error_norm_list = []
    ...
    def create_sample_model(self, args):
        ...
        if args.model == 'conv4':
            return Conv_Model(args.imgc, args.num_ways, args.update, args.filter_size, feature_size, drop_p=args.dropout).cuda()
        else:
