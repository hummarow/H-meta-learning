...
class ImgClassification:
    ...
    def __init__(self, args):
        ...
        assert (self.k_shot + self.k_query) <= 20  # 为什么呢？ 每个类别下的图片数量为 20
    ...
    def construct_task(self, data_pack, task_num):
        ...
        x_spt_task_cluster, x_qry_task_cluster, y_spt_task_cluster, y_qry_task_cluster = [], [], [], []
    ...
    def unpack_hie_data(self, eight_cluster_batch, train_or_test):
        ...
        eight_cluster_mass[:4] = [
            one_cluster_batch.reshape((-1, one_cluster_batch.shape[2], self.channel, self.resize, self.resize)) for
            one_cluster_batch in eight_cluster_batch[:4]]
        eight_cluster_mass[4:] = [one_cluster_batch.reshape((-1, one_cluster_batch.shape[2])) for one_cluster_batch
                                  in eight_cluster_batch[4:]]
    ...
    def load_data_cache_hie(self, data_pack: numpy.ndarray, mode, sample_mode):
        ...
        for sample in range(1):  # 缓存为1，要不然显存受不了
            x_spt_task_spt_set_batch, x_spt_task_qry_set_batch, x_qry_task_spt_set_batch, x_qry_task_qry_set_batch, y_spt_task_spt_set_batch, y_spt_task_qry_set_batch, y_qry_task_spt_set_batch, y_qry_task_qry_set_batch = [], [], [], [], [], [], [], []
            ...
            if self.dataset == 'domainnet':
                if data_pack.shape[0] >= self.task_cluster_batch_num:
                    domain_selected = np.random.choice(range(data_pack.shape[0]), self.task_cluster_batch_num,
                                                       replace=False)
                else:
                    domain_selected = np.random.choice(range(data_pack.shape[0]), self.task_cluster_batch_num,
                                                       replace=True)
                ...
                for i in range(self.task_cluster_batch_num):
                    ...
                    if mode == 'train':
                        if bool(self.if_val):
                            ...
                            spt_pack = data_pack[index_selected][: self.val_split]  # 如果是train,传入的data_pack则为前1200个类
                        ...
                        if bool(self.if_val):
                            ...
                            spt_pack = data_pack[index_selected][
                                       : self.val_split]  # 如果是test，传入的则是训练集的前800个和测试集的400多个的拼接
                            qry_pack = data_pack[index_selected][self.val_split:]

                        elif not bool(self.if_val) and self.test_spt_task_from == 'meta_train':
                    ...
                    if sample_mode == 'hierarchical_concentrate':
                        sub_index = random.sample(range(len(spt_pack)), self.candidate_num)
                        spt_pack = spt_pack[sub_index]
                        sub_index = random.sample(range(len(qry_pack)), self.candidate_num)
                ...
                for i in range(self.task_cluster_batch_num):  # one batch means one set
                    ...
                    if mode == 'train':
                        if bool(self.if_val):
                            ...
                            spt_pack = data_pack[: self.val_split]  # 如果是train,传入的data_pack则为前1200个类
                        ...
                        if bool(self.if_val):
                            ...
                            spt_pack = data_pack[: self.val_split]  # 如果是test，传入的则是训练集的前800个和测试集的400多个的拼接
                            qry_pack = data_pack[self.val_split:]
            ...
            x_spt_task_spt_set_batch = np.asarray(x_spt_task_spt_set_batch).astype(np.float32)
            x_spt_task_qry_set_batch = np.asarray(x_spt_task_qry_set_batch).astype(np.float32)
            x_qry_task_spt_set_batch = np.asarray(x_qry_task_spt_set_batch).astype(np.float32)
            x_qry_task_qry_set_batch = np.asarray(x_qry_task_qry_set_batch).astype(np.float32)
            ...
            data_cache.append(
                [x_spt_task_spt_set_batch, x_spt_task_qry_set_batch, x_qry_task_spt_set_batch, x_qry_task_qry_set_batch,
    ...
    def load_data_cache(self, data_pack):
        ...
        for sample in range(10):  # num of episodes
            ...
            # [b, setsz, 1, 84, 84]
            x_spts = np.array(x_spts).astype(np.float32).reshape(self.task_num, setsz, 1, self.resize, self.resize)
            y_spts = np.array(y_spts).astype(np.int).reshape(self.task_num, setsz)
            # [b, qrysz, 1, 84, 84]
            x_qrys = np.array(x_qrys).astype(np.float32).reshape(self.task_num, querysz, 1, self.resize, self.resize)
    ...
    def next_hierarchical(self, mode='train', sample_mode=""):
        ...
        if self.indexes[mode] >= len(self.datasets_cache[mode]):
            ...
            if sample_mode == 'hierarchical_uniform':
                if mode == 'train':
                    self.datasets_cache[mode] = self.load_data_cache_hie(self.datasets[mode], mode, sample_mode)
                else:
                    if bool(self.if_val):
                        ...
                        if self.dataset != 'domainnet':
                            self.datasets_cache[mode] = self.load_data_cache_hie(np.vstack((
                                self.datasets['train'][: self.val_split], self.datasets[mode])), mode,
                                sample_mode)  # 前800是spt_task，后面的是qry_tak
                        else:
                            temp_list = []

                            assert self.datasets['train'].shape[0] == self.datasets[mode].shape[0]

                            for i in range(self.datasets['train'].shape[0]):
                                temp = np.vstack((self.datasets['train'][i][: self.val_split], self.datasets[mode][i]))
                                temp_list.append(temp)

                            temp_array = np.array(temp_list)
                            # print(temp_array.shape)
                            self.datasets_cache[mode] = self.load_data_cache_hie(temp_array, mode, sample_mode)
                        ...
                        if self.dataset != 'domainnet':
                            self.datasets_cache[mode] = self.load_data_cache_hie(
                                np.vstack((self.datasets['train'], self.datasets[mode])), mode, sample_mode)
                        else:
                            temp_list = []
                            ...
                            for i in range(self.datasets['train'].shape[0]):
                                temp = np.vstack((self.datasets['train'][i], self.datasets[mode][i]))
                                temp_list.append(temp)
                ...
                assert sample_mode == 'hierarchical_concentrate'  # hierarchical_concentrate
        ...
        if not os.path.isfile(os.path.join(root, 'omniglot.npy')):
            ...
            for (img, label) in x:
                ...
                if label in temp.keys():
                    ...
                    # t = np.array(temp[label])
                    # print(t.shape)  # (2, 1, 64, 64)
                    # 这是四维的
            ...
            x = np.load(os.path.join(root, 'omniglot.npy'))
    ...
    def create_mini_imagenet(self, root):
        # temp = root
        # root = './'
        if not os.path.isfile(os.path.join(root, 'miniimagenet.npy')):
            # root = temp

            mode = 'all'

            # label -> [filename1, filename2, ...]  标签到文件列表的映射，标签即为图片类别
            csvdata: dict = self.loadCSV(os.path.join(root, mode + '.csv'))  # csv path
            ...
            for i, (k, v) in enumerate(csvdata.items()):
                ...
                img2label[k] = i + startidx  # {"img_name[:9]":label}   # 这里为什么要加上一个 startidx
            ...
            print('write into miniimagenet.npy.')
        else:
            x = np.load(os.path.join(root, 'miniimagenet.npy'))
            print('load from miniimagenet.npy.')
    ...
    def create_domainnet(self, root):
        ...
        if not os.path.isfile(save_path):
            ...
            domain_list = os.listdir(data_path)
            ...
            for domain in domain_list:
                class_path = os.path.join(data_path, domain, domain.lower())
                ...
                for class_name in class_list:
                    ...
                    # 如果不存在或者更小
                    if class_name not in class_to_min_num or len(image_list) < class_to_min_num[class_name]:
            ...
            class_to_min_num_sorted = dict(sorted(class_to_min_num.items(), key=lambda x: x[1]))
