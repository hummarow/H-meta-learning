...
class H_ProtoNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        assert self.task_cluster_batch_num == 1 and self.task_cluster_batch_num_test == 1  # 有必要batch吗,先写无batch版
        ...
        self.second_optim = torch.optim.Adam(self.second_map.parameters(), lr=self.middle_lr)

        self.reset = args.reset_second_map

        self.dataset = args.dataset

    def learning_on_the_high_level_task(self, mode, task_num, x_spt, x_qry, y_spt, y_qry):
        q_loss_sum = 0
        q_acc_sum = 0
    ...
    def forward(self, eight_cluster_batch, train_or_test):  # 快更新first_map，慢更新second_map效果比反过来好
        for i in range(self.task_cluster_batch_num): # task_cluster_batch_num=1
            ...
            if self.reset:
                ...
                self.first_optim = torch.optim.Adam(self.first_map.parameters(), lr=self.middle_lr)

            # learning on T_S
            self.learning_on_the_high_level_task('T_S', self.task_spt, x_spt_task_spt_set, x_spt_task_qry_set,
                                                 y_spt_task_spt_set, y_spt_task_qry_set)

            self.second_optim.zero_grad()
            q_loss_avg, q_acc_avg = self.learning_on_the_high_level_task('T_Q', self.task_qry, x_qry_task_spt_set,
                                                                         x_qry_task_qry_set,
                                                                         y_qry_task_spt_set, y_qry_task_qry_set)
