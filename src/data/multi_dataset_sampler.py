...
class MultipleDataset(Dataset):
    def __init__(self, dataset_dict):
        self.cluster_info = (
            []
        )  # index -> (ds_name, ds_info, sample_start, sample_end, label_start, label_end)
        self.labels = None
...
class MultipleDatasetSampler(object):
    def __init__(
        ...
        contrastive=False,
        batch=2,  # 2 for support and query TASK
    ):
        """
        ...
        self.num_clusters = len(multiple_datasets.cluster_info)
        self.cluster_samples = np.tile(
            np.arange(self.num_clusters),
            self.total_steps * batch,
        )
        ...
        for i in range(max(labels) + 1):
            ids = np.argwhere(labels == i).reshape(
                -1
            )  # output shape of "np.argwhere" is column array
    ...
    def __len__(self):
        return len(self.cluster_samples)
        # if self.is_train:
    ...
    def __iter__(self):
        ...
        for i_batch in range(self.total_steps):
            batch = []
            for i_domain in range(self.num_clusters):
                index = i_batch * self.num_clusters + i_domain
                if self.is_random_classes:
                    class_ids = np.random.choice(
                        self.labels_num, self.n_way, replace=False
                    )
                else:
                    cluster_id = self.cluster_samples[index]
                    class_ids = np.random.choice(
                        np.arange(
                            self.multiple_datasets.cluster_info[cluster_id][4],
                            self.multiple_datasets.cluster_info[cluster_id][5],
                        ),
                        self.n_way,
                        replace=self.contrastive,  # n_way k_shot 방식도 self.contrastive따라서 바꿔야
                    )
                for class_id in class_ids:
                    instances_ids = self.label_2_instance_ids[class_id]
                    instances_ids_selected = torch.randperm(len(instances_ids))[
                        0 : self.k_shot
                    ]
                    # NOTE: temporal fix.
                    if len(instances_ids_selected) < self.k_shot:
                        instances_ids_selected = torch.cat(
                            (
                                instances_ids_selected,
                                torch.multinomial(
                                    torch.ones(len(instances_ids)),
                                    self.k_shot - len(instances_ids_selected),
                                    replacement=True,
                                ),
                            )
                        )
